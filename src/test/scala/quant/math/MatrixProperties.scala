...
class MatrixProperties extends munit.ScalaCheckSuite {
  def genSquareSizeMatrix[A: Arbitrary]: Gen[Matrix[A]] = for {
    size <- Gen.choose(1, 32)
    m <- Gen
      .listOfN(size, Gen.listOfN(size, Arbitrary.arbitrary[A]).map(_.toVector))
      .map(_.toVector)
      .map(Matrix.apply)
  } yield m

  property("isSquare") {
    forAll(genSquareSizeMatrix[Complex[Real]]) { m =>
      m.isSquare
    } && forAll(
      Arbs.arbMtrx[Complex[Real]].arbitrary.suchThat(m => m.rows != m.cols)
    ) { m =>
      !m.isSquare
    }
  }
  ...
  property("scalar mult") {
    forAll { (m: Matrix[Complex[Real]], s: Complex[Real]) =>
      val multed = s *: m
      (m.zip(multed)).forall { case (vm, vmult) =>
        s * vm === vmult
      }
    }
  }
  ...
  property("unitary 1x1") {
    forAll(genUnitaryComplexRealMatrix1x1) { m =>
      Matrix.isUnitary(m)
    }
  }
  ...
  property("zeros") {
    forAll(Gen.choose(1, 32), Gen.choose(1, 32)) { (i, j) =>
  ...
  property("ones") {
    forAll(Gen.choose(1, 32)) { i =>
      Matrix.eye[Complex[Real]](i).zipWithIndex.forall { case (v, r, c) =>
        r == c && v === Complex.one[Real] || r != c && v === Complex.zero[Real]
      }
    }
  }
}
