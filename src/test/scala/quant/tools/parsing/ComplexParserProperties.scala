...
class ComplexParserProperties extends munit.ScalaCheckSuite {
  private def parseResultAssert[T](
      pr: ParseResult[T]
  )(f: T => Boolean): Boolean =
    pr match {
      case ParseResult.Done(_, result) => f(result)
      case ParseResult.Fail(_, _, message) =>
        println(s"Parsing failed: $message"); false
      case _ => false
    }
  ...
  property("number") {
    forAll(genDouble) { d =>
      parseResultAssert(number.parse(d.toString).done)(parsed =>
        Real(d) === parsed
      )
    }
  }
  ...
  property("pi") {
    List("pi", "pI", "Pi", "PI").forall(s =>
      parseResultAssert(number.parse(s).done) { res =>
        res === Real.pi
      }
    )
  }
  ...
  property("dexpr") {
    forAll(genRexpr) { case (s, f) =>
      parseResultAssert(rexpr.parse(s).done)(f)
    }
  }

  property("dexpr.minus") {
    forAll(genDouble) { d =>
  ...
  property("dexpr.mult") {
    forAll(genDouble, genDouble) { (d1, d2) =>
      parseResultAssert(rexpr.parse(s"$d1 * $d2").done)(parsed =>
        Real(d1) * Real(d2) === parsed
      )
    }
  }

  property("dexpr.div") {
    forAll(genDouble, genDouble.suchThat(_ != 0.0)) { (d1, d2) =>
      parseResultAssert(rexpr.parse(s"$d1 / $d2").done)(parsed =>
        Real(d1) / Real(d2) === parsed
      )
    }
  ...
  property("complex.single") {
    forAll(genDouble) { r =>
      parseResultAssert(complex.parse(r.toString).done)(parsed =>
        Complex(Real(r)) === parsed
      )
    }
  }

  property("complex.both") {
    forAll(genDouble, genDouble) { (r, i) =>
      parseResultAssert(complex.parse(s"($r, $i)").done)(parsed =>
        Complex(Real(r), Real(i)) === parsed
      )
    }
  }
  ...
  property("row.single") {
    forAll(genRow) { r =>
      parseResultAssert(row.parse(r.mkString(" ")).done)(parsed =>
        r.map(v => Complex(Real(v))) === parsed
      )
    }
  }
  ...
  property("matrix.both") {
    forAll(genMatBoth) { m =>
      val result = parseMatrix(m.map(_.map { case (d1, d2) =>
        s"($d1, $d2)"
      }.mkString(" ")))
      val expected = Matrix(m.toVector.map(_.map { case (d1, d2) =>
        Complex(Real(d1), Real(d2))
      }))
      result.exists(r =>
        (r.zip(expected)).forall { case (rv, ev) =>
  ...
  property("matrix.concrete") {
    ...
    // Sanity
    require(
      n == ExampleMatrices.parserMatrices.length,
    ...
    val input = (
      ...
      ExampleMatrices.parserMatrices
    ).parTupled
