...
trait MatrixInstances {
  ...
  implicit def matrixAdditiveCommutativeSemigroupInstance[A: AdditiveCommutativeSemigroup]: AdditiveCommutativeSemigroup[Matrix[A]] =
    (x: Matrix[A], y: Matrix[A]) => {
      ...
      Matrix((x.m zip y.m).map {
        case (r1, r2) => (r1 zip r2).map {
          case (e1, e2) => e1 + e2
        }
      })
    }

  implicit def matrixMultiplicativeSemigroupInstance[A: MultiplicativeSemigroup : AdditiveCommutativeSemigroup]: MultiplicativeSemigroup[Matrix[A]] =
    (x: Matrix[A], y: Matrix[A]) => {
      // FIXME: dirty, may throw an exception
      require(x.cols === y.rows,
        s"First matrix columns was not equal second matrix rows: ${x.cols} and ${x.rows}")
      Matrix(x.m.map(r => y.m.transpose.map(c => (r zip c).map {
        case (rv, cv) => rv * cv
      }.reduce(_ + _))))
    }
  ...
  implicit val matrixFoldableInstance: Foldable[Matrix] = new Foldable[Matrix] {
    ...
    override def foldRight[A, B](fa: Matrix[A], lb: Eval[B])(f: (A, Eval[B]) => Eval[B]): Eval[B] =
