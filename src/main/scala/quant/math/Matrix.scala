...
import algebra.ring.{AdditiveMonoid, CommutativeRing, MultiplicativeSemigroup, Rig}
...
class Matrix[T](private[quant] val m: Vector[Vector[T]]) extends Serializable {
  ...
  def zipWith[S, R](that: Matrix[S])(f: (T, S) => R): Matrix[R] =
    Matrix((m, that.m).parMapN {
      case (r1, r2) => (r1, r2).parMapN(f)
    })

  def zipWithIndex[A]: Matrix[(T, Int, Int)] =
    Matrix(m.zipWithIndex.map {
      case (r, ri) => r.zipWithIndex.map {
        case (v, ci) => (v, ri, ci)
      }
    })
}
...
object Matrix {
  ...
  def eye[T: Rig](size: Int): Matrix[T] =
    new Matrix(Vector.tabulate(size, size)((r, c) => if (r === c) Rig[T].one else Rig[T].zero))

  def zeros[T: AdditiveMonoid](rows: Int, cols: Int): Matrix[T] =
    new Matrix[T](Vector.tabulate(rows, cols)((_, _) => AdditiveMonoid[T].zero))

  def isUnitary[T: CommutativeRing : Eq : Signed](m: Matrix[Complex[T]]): Boolean =
    (m * m.map(_.conjugate).transpose) === eye[Complex[T]](m.rows)

  // TODO: rep should be 1x2
  private def vReplaceSquareAt[T](v: Vector[T])(j1: Int, j2: Int, rep: Vector[T]): Vector[T] = {
    require(rep.size === 2, s"Replacement vector should have size 2, got: ${rep.size}")
    v.zipWithIndex.foldLeft(Vector.empty[T]) {
      case (vt, (t, ind)) =>
        if (ind === j1)
          vt :+ rep(0)
        else if (ind === j2)
  ...
  private def vvReplaceSquareAt[T](vv: Matrix[T])(i1: Int, i2: Int, rep: Matrix[T]): Matrix[T] = {
    require(rep.cols === 2 && rep.rows === 2, s"Replacement matrix should 2x2, got: ${rep.rows}x${rep.cols}")
