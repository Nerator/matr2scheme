...
object ComplexParser {
  ...
  def rexpr: Parser[Real] = {
    (char('-') ~> rexpr).map(-_) |
      ...
      (number, token(rexpr1)).mapN { case (n, de1) =>
        de1(n)
      }
  }
  ...
  def complex: Parser[Complex[Real]] =
    myParens(
      (token(rexpr), token(char(',')) ~> token(rexpr)).mapN(Complex.apply)
    ) |
      rexpr.map(Complex(_))
    ...
    ss.toVector
      .map(l =>
        row.parse(l.trim).done match {
          case ParseResult.Fail(input, stack, message) =>
            s"Parsing failed. Input: $input, stack: $stack, message: $message".asLeft
