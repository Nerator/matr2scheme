...
object Runner extends IOApp {
  override def run(args: List[String]): IO[ExitCode] = for {
    ...
    code <- checkArgsLength(args).fold(
      ...
      _ => {
        val matrixAndResults = runChecks(args.head).semiflatMap(m => runAlgs(m).map {
          case (r1, r2) => (m, r1, r2)
        })
        val printResAndCheckProperty = matrixAndResults.semiflatMap {
          case (matrix, r1, r2) => for {
  ...
  def runChecks(fileName: String): EitherT[IO, String, Matrix[Complex[Real]]] = {
    for {
  ...
  def checkIfFileExists(filename: String): Either[String, Unit] =
    Either.cond(new File(filename).exists, (), s"""файл "$filename" не найден.""")
  ...
  def readData(filename: String): EitherT[IO, String, Matrix[Complex[Real]]] = {
    val res: Resource[IO, BufferedSource] = Resource.make(IO(Source.fromFile(filename))) {
      s => IO(s.close()).handleErrorWith(t => println(s"Ошибка при закрытии ресурса: ${t.getMessage}").pure[IO])
    }
    val abc = res.use { bs =>
  ...
  def checkInputData(matrix: Matrix[Complex[Real]]): Either[String, Matrix[Complex[Real]]] =
    matrix.asRight
      .ensure("неквадратная матрица")(_.isSquare)
      .ensure("неунитарная матрица")(Matrix.isUnitary)

  def runAlgs(matrix: Matrix[Complex[Real]]): IO[(List[Matrix[Complex[Real]]], List[Matrix[Complex[Real]]])] = {
    val r1 = Razl.algNielsenChang(matrix)
    val r2 = Razl.algNakaharaOhmi(matrix)
    IO((r1, r2))
  }

  def checkMatrixProductCloseToMatrix(m: Matrix[Complex[Real]], ms: List[Matrix[Complex[Real]]]): IO[Unit] = {
  ...
  def printRes(res: List[Matrix[Complex[Real]]], method: String): IO[Unit] = for {
